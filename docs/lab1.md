...
| `ProductCD` | categorical | Тип продукта/канала: W (Web), H (Hotel/Travel), C (Charge/CNP), S (Service), R (Retail) |
...
Бизнес-метрики антифрод-системы оценивают эффективность борьбы с мошенничеством через призму финансовых результатов и клиентского опыта.

#### Финансовые метрики

| Метрика | Формула / Описание | Целевое значение |
|---|---|---|
| **Net Savings (Чистая экономия)** | Сумма предотвращённого фрода − ущерб от ложных блокировок (упущенная выгода + операционные расходы) | Максимизация |
| **Chargeback Rate (Доля чарджбэков)** | Сумма/количество чарджбэков / Общий объём транзакций × 100% | < 0.1% |
| **Fraud Loss Ratio (Доля потерь от фрода)** | Финансовые потери от фрода / Общая выручка × 100% | < 0.05% |
| **ROI антифрод-системы** | Предотвращённый ущерб / Стоимость внедрения и поддержки системы | > 5× |

#### Метрики клиентского опыта

| Метрика | Описание | Целевое значение |
|---|---|---|
| **False Positive Rate (FPR)** | Доля легитимных транзакций, ошибочно заблокированных | < 0.1% |
| **Среднее время детектирования** | Время от поступления транзакции до вынесения решения | < 200 мс |

**Влияние качества модели на бизнес-метрики:**
- Высокий **Recall** (↑ обнаружение фрода) → ↑ Net Savings, ↓ Fraud Loss Ratio, ↓ Chargeback Rate — система перехватывает больше мошеннических операций, напрямую сокращая финансовые потери
- Высокий **Precision** (↓ ложные срабатывания) → ↓ FPR, ↑ Net Savings — меньше легитимных транзакций блокируется, снижается упущенная выгода и нагрузка на службу поддержки
- Баланс Precision/Recall → оптимальный **ROI**: слишком агрессивная модель увеличивает операционные расходы на разбор ложных блокировок; слишком мягкая — пропускает фрод и снижает Net Savings
- Низкая **Latency** → возможность блокировки транзакции в реальном времени до списания средств
...
> Анализ выполнен на полном датасете `train_transaction.csv` (590 540 строк, скрипт: [`Preproc/eda.py`](../Preproc/eda.py)).

#### Дисбаланс классов

| Класс | Количество | Доля |
|---|---|---|
| 0 — Легитимные | 569 877 | **96.50%** |
| 1 — Мошеннические | 20 663 | **3.50%** |
| **Итого** | **590 540** | **100%** |

![Распределение классов и фрод-рейт по ProductCD](images/eda_class_distribution.png)

> Сильный дисбаланс классов (~1:28) требует применения `class_weight='balanced'` или SMOTE при обучении.

#### Анализ суммы транзакции (`TransactionAmt`)

| Статистика | Легитимные (0) | Мошеннические (1) |
|---|---|---|
| Количество | 569 877 | 20 663 |
| Среднее | 134.51 USD | 149.24 USD |
| Медиана | 68.50 USD | 75.00 USD |
| Std | 239.40 | 232.21 |
| Мин | 0.25 | 0.29 |
| Макс | 31 937.39 | 5 191.00 |

![Распределение TransactionAmt](images/eda_transaction_amt.png)

> Распределение сильно правостороннее — требуется `log1p`-трансформация. Мошеннические транзакции в среднем на ~11% крупнее легитимных; у легитимных встречаются значительно большие выбросы (макс. ~32k USD).

#### Фрод-рейт по типу продукта (`ProductCD`)

`ProductCD` — код типа продукта/канала транзакции в системе Vesta:

| ProductCD | Расшифровка | Всего транзакций | Мошеннических | Фрод-рейт |
|---|---|---|---|---|
| **C** | Charge (прямое списание / CNP-транзакция без физической карты) | 68 519 | 8 008 | **11.69%** |
| **S** | Service (оплата сервисов / подписок) | 11 628 | 686 | 5.90% |
| **H** | Hotel / Travel (бронирование, туристические услуги) | 33 024 | 1 574 | 4.77% |
| **R** | Retail (розничная торговля) | 37 699 | 1 426 | 3.78% |
| **W** | Web (интернет-покупки, e-commerce) | 439 670 | 8 969 | 2.04% |

> Категория **C** (CNP-транзакции без физической карты) имеет фрод-рейт в 5.7× выше категории W — сильный дискриминирующий признак для модели. CNP-транзакции традиционно наиболее уязвимы к мошенничеству, так как не требуют физического присутствия карты.

#### Пропущенные значения и выбросы

| Проблема | Признаки | Стратегия обработки |
|---|---|---|
| Пропуски (до 50%) | `id_12`–`id_38`, `DeviceType`, `addr1`, `P_emaildomain` | Медиана (числовые), `"unknown"` (категориальные) |
| Выбросы | `TransactionAmt` | `log1p`-трансформация или `RobustScaler` |

**Вывод по EDA:** Датасет содержит интерпретируемые признаки, пригодные для ручного ввода в веб-форму. Ключевые наблюдения: дисбаланс классов ~1:28 (3.5% фрода), категория `ProductCD=C` — главный индикатор фрода (11.69%), `TransactionAmt` требует log-трансформации. Основные задачи предобработки: объединение таблиц по `TransactionID`, импутация пропусков, кодирование категориальных признаков, нормализация числовых признаков.
